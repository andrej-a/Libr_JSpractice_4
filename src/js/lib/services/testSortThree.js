...
function sortByFeedbacks(arr) {
    const temp = JSON.parse(JSON.stringify(arr));//глубокое копирование объекта чтобы не трогат исходный
    ...
    btnFeed.addEventListener("click", function() {
        ...
        temp.forEach(item => {
            ...
            <br> 
            Телефон ${item.name}, 
            <b>${item.ratingRevievs} отзывов</b> , 
            стоимость ${item.price.newUan ? item.price.newUan : item.price} 
            <br>`;
        });
    });
...
sortByFeedbacks(thingsArray);


function sortByprice(arr) {
    const temp = JSON.parse(JSON.stringify(arr));//глубокое копирование объекта чтобы не трогать исходный
    const parent = document.querySelector(".parent");
    const btnCost = document.querySelector("#cost");
    temp.forEach(item => {//все перебираем и приводим к одному значению
        item.price = item.price.newUan ? +item.price.newUan.replace(/\D/g, "") : +item.price.replace(/\D/g, "");
    });

    temp.sort((a, b) => a.price < b.price ? 1 : -1);//сортируем. внутри sort алгоритм быстрой сортировки
    ...
    btnCost.addEventListener("click", function() {
        ...
        temp.forEach(item => {
            ...
            <br> 
            Телефон ${item.name}, 
            <b>${item.ratingRevievs}</b> отзывов, 
            стоимость <b>${item.price} UAH</b> 
            <br>`;
        });
    });
...
sortByprice(thingsArray);




//console.log(thingsArray[center]["ratingRevievs"].replace(/\D/g, ""));